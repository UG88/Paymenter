...
class Pterodactyl extends Server
{
    ...
    public function getProductConfig($values = []): array
    {
        ...
        return [
            ...
            [
                'name' => 'server_expiry_api_key',
                'label' => 'Server Expiry API Parameter',
                'description' => 'The API request parameter key used by your Pterodactyl Server Expiry addon (e.g. expires_at or expiry_date). Leave empty to disable.',
                'type' => 'text',
                'required' => false,
            ],
        ];
    ...
    public function createServer(Service $service, $settings, $properties)
    {
        ...
        $expiryDate = $service->expires_at ? ($service->expires_at instanceof \DateTimeInterface ? $service->expires_at->format('Y-m-d') : date('Y-m-d', strtotime((string)$service->expires_at))) : 'Never';
        $environment = [];
        foreach ($eggData['attributes']['relationships']['variables']['data'] as $variable) {
            $envKey = $variable['attributes']['env_variable'];
            $environment[$envKey] = $settings[$envKey] ?? $variable['attributes']['default_value'];
        }
        ...
        $apiExpiryKey = $settings['server_expiry_api_key'] ?? null;
        if ($apiExpiryKey) {
            $serverCreationData[$apiExpiryKey] = $service->expires_at ? ($service->expires_at instanceof \DateTimeInterface ? $service->expires_at->format('Y-m-d H:i:s') : date('Y-m-d H:i:s', strtotime((string)$service->expires_at))) : null;
        }
    ...
    public function upgradeServer(Service $service, $settings, $properties)
    {
        ...
        foreach ($eggData['attributes']['relationships']['variables']['data'] as $variable) {
            $envKey = $variable['attributes']['env_variable'];
            if (isset($server['attributes']['container']['environment'][$envKey])) {
                $environment[$envKey] = $server['attributes']['container']['environment'][$envKey];
            } else {
                $environment[$envKey] = $settings[$envKey] ?? $variable['attributes']['default_value'];
            }
        }
        ...
        if ($apiExpiryKey) {
            try {
                $this->request('/api/application/servers/' . $server['attributes']['id'] . '/details', 'patch', [
                    'name' => $server['attributes']['name'],
                    'user' => $server['attributes']['user'],
                    $apiExpiryKey => $service->expires_at ? ($service->expires_at instanceof \DateTimeInterface ? $service->expires_at->format('Y-m-d H:i:s') : date('Y-m-d H:i:s', strtotime((string)$service->expires_at))) : null,
                ]);
            } catch (Exception $e) {
                // Ignore failure if details update fails, ensuring main build/startup changes succeed
            }
        }
